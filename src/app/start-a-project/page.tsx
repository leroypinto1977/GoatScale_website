            ...
            <AnimatePresence mode="wait" custom={dir}>
              {step === 1 && (
                <motion.div key="s1" custom={dir} variants={slideV} initial="enter" animate="center" exit="exit" transition={{ duration: 0.35, ease:[0.16,1,0.3,1] }} className={styles.stepContent}>
                  ...
                  <div className={styles.fieldGroup}>
                    <div className={styles.field}><label className={styles.label}>Your Name *</label><input className={styles.input} type="text" placeholder="Alex Johnson" value={form.name} onChange={e => set('name', e.target.value)} /></div>
                    <div className={styles.field}><label className={styles.label}>Company</label><input className={styles.input} type="text" placeholder="Acme Inc. (optional)" value={form.company} onChange={e => set('company', e.target.value)} /></div>
                    <div className={styles.field}><label className={styles.label}>Email *</label><input className={styles.input} type="email" placeholder="alex@company.com" value={form.email} onChange={e => set('email', e.target.value)} /></div>
                    <div className={styles.field}><label className={styles.label}>Phone (optional)</label><input className={styles.input} type="tel" placeholder="+91 98765 43210" value={form.phone} onChange={e => set('phone', e.target.value)} /></div>
                  </div>
                ...
                <motion.div key="s2" custom={dir} variants={slideV} initial="enter" animate="center" exit="exit" transition={{ duration: 0.35, ease:[0.16,1,0.3,1] }} className={styles.stepContent}>
                  ...
                  <div className={styles.field}>
                    ...
                    <div className={styles.typeGrid}>
                      {PROJECT_TYPES.map(t => (
                        <button key={t.id} type="button" className={`${styles.typeCard} ${form.projectType === t.id ? styles.typeActive : ''}`} onClick={() => set('projectType', t.id)}>
                          <span className={styles.typeIcon}>{t.icon}</span><span>{t.label}</span>
                        </button>
                      ))}
                    </div>
                  </div>
                  <div className={styles.twoCol}>
                    <div className={styles.field}><label className={styles.label}>Budget</label><select className={styles.select} value={form.budget} onChange={e => set('budget', e.target.value)}><option value="">Select budget</option>{BUDGET_OPTIONS.map(b => <option key={b} value={b}>{b}</option>)}</select></div>
                    <div className={styles.field}><label className={styles.label}>Timeline</label><select className={styles.select} value={form.timeline} onChange={e => set('timeline', e.target.value)}><option value="">Select timeline</option>{TIMELINE_OPTIONS.map(t => <option key={t} value={t}>{t}</option>)}</select></div>
                  </div>
              ...
              {step === 3 && (
                <motion.div key="s3" custom={dir} variants={slideV} initial="enter" animate="center" exit="exit" transition={{ duration: 0.35, ease:[0.16,1,0.3,1] }} className={styles.stepContent}>
                  ...
                  <div className={styles.field}><label className={styles.label}>Describe your project *</label><textarea className={styles.textarea} rows={5} placeholder="What problem are you solving? Who are your users? Any references you love?" value={form.brief} onChange={e => set('brief', e.target.value)} /></div>
                  <div className={styles.field}><label className={styles.label}>How did you find us?</label><select className={styles.select} value={form.how} onChange={e => set('how', e.target.value)}><option value="">Select one</option>{HOW_OPTIONS.map(h => <option key={h} value={h}>{h}</option>)}</select></div>
                  <div className={styles.btnRow}>
              ...
              {step === 4 && (
                <motion.div key="s4" custom={dir} variants={slideV} initial="enter" animate="center" exit="exit" transition={{ duration: 0.35, ease:[0.16,1,0.3,1] }} className={styles.stepContent}>
                  ...
                  <div className={styles.btnRow}>
                    <button type="button" className="btn btn-secondary" onClick={() => goTo(3)}>← Edit</button>
                    <button type="button" className="btn btn-primary" onClick={() => setSubmitted(true)}>Send Brief ✓</button>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
        ...
        <div className={styles.infoSide}>
          <motion.div initial={{ opacity: 0, x: 32 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.2 }} className={styles.infoBox}>
            ...
            <div className={styles.infoMeta}>
              ...
              <span>🔒 Brief stays confidential</span>
            </div>
          </motion.div>
        </div>
      </div>
